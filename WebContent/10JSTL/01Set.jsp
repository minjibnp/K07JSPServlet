...
<body>
	...
	<h3>JSP코드로 출력</h3>
	<!-- 영역에 저장될때 Object타입으로 자동형변환되므로
		사용시 해당 자료형으로 캐스팅해야한다. -->
	<ul>
		<li>아이디:
	...
	<!-- EL로 출력시에는 별도의 형변환이 필요없고, 속성명이 중복되지
		않는다면 xxScope계열의 내장객체를 생략할 수 있다. -->
	<h3>EL로 출력</h3>
	<ul>
	...
	property : 값을 지정하는 속성. 표현식, EL, 값 모두 가능함.
			-자바빈인 경우:속성명(멤버변수)
			-Map컬렉션인 경우 : key값
	     ※scope속성은 var 속성이 지정된 태그에서만 지정가능.
	...
	<c:set target="${list[0] }" property="id" value="EXO"/>
	<c:set target="${list[0] }" property="pass" value="0000"/>
	<c:set target="${list[0] }" property="name" value="엑소"/>
	...
	<c:set target="${map.args }" property="id" value="오마이걸"/>
	<c:set target="${map.args }" property="pass" value="7777"/>
	<c:set target="${map.args }" property="name" value="유아"/>
	
	<h4>속성값 변경후 출력</h4>
	<ul>
		<li>아이디:${map.args.id }</li>
		<li>비번:${map.args.pass }</li>
		<li>이름:아이디:${map.args.name }</li>
	</ul>
</body>
</html>